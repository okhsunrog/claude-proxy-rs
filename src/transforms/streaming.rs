...
//! - `stream_anthropic_to_openai`: Convert Anthropic SSE to OpenAI SSE format
//! - `stream_anthropic_to_openai_with_usage`: Same as above but records token usage
//! - `stream_strip_mcp_prefix`: Strip mcp_ prefix from native Anthropic SSE
//! - `stream_strip_mcp_prefix_with_usage`: Same as above but records token usage
//!
...
#[allow(dead_code)]
pub fn stream_anthropic_to_openai(
...
///
/// Same as `stream_anthropic_to_openai` but records token usage to the client keys store.
...
/// Strip mcp_ prefix from tool names in native Anthropic SSE stream.
///
...
/// Includes keep-alive pings every 15 seconds to prevent connection timeouts.
#[allow(dead_code)]
pub fn stream_strip_mcp_prefix(
    body: impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static,
) -> impl Stream<Item = Result<Bytes, std::io::Error>> + Send {
    use futures_util::StreamExt;
...
///
/// Same as `stream_strip_mcp_prefix` but records token usage to the client keys store.
